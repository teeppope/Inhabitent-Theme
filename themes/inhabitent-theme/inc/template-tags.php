 ...
 */
  ...
  */
 function inhabitent_theme_posted_on() {
  ...
  */
 function inhabitent_theme_posted_by() {
 	$byline = sprintf(
 		esc_html( 'by %s' ),
  ...
  */
 function inhabitent_theme_comment_count() {
  ...
  */
 function inhabitent_theme_entry_footer() {
 	// Hide category and tag text for pages.
 	if ( 'post' === get_post_type() ) {
 		/* translators: used between list items, there is a space after the comma */
 		$categories_list = get_the_category_list( esc_html( ', ' ) );
 		if ( $categories_list && inhabitent_theme_categorized_blog() ) {
  ...
  * @return bool
  */
 function inhabitent_theme_categorized_blog() {
 	if ( false === ( $all_the_cool_cats = get_transient( 'inhabitent_theme_categories' ) ) ) {
 		...
 		set_transient( 'inhabitent_theme_categories', $all_the_cool_cats );
 	}

 	if ( $all_the_cool_cats > 1 ) {
 		// This blog has more than 1 category so inhabitent_theme_categorized_blog should return true.
 		return true;
 	} else {
 		// This blog has only 1 category so inhabitent_theme_categorized_blog should return false.
 		return false;
 	}
 }

 /**
  * Flush out the transients used in inhabitent_theme_categorized_blog.
  */
 function inhabitent_theme_category_transient_flusher() {
 	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
 		return;
 	}
 	// Like, beat it. Dig?
 	delete_transient( 'inhabitent_theme_categories' );
 }
 add_action( 'edit_category', 'inhabitent_theme_category_transient_flusher' );
 add_action( 'save_post',     'inhabitent_theme_category_transient_flusher' );
  ...
  */
 function inhabitent_theme_comment_list( $comment, $args, $depth ) {
  ...
  */
 function inhabitent_theme_numbered_pagination() {
 	global $wp_query;
 	$big = 999999999;
